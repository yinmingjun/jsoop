...
jsoop regard itself as an global module. The first required jsoop module publish the rtti
storge for all other module(since v0.3). And also publish global jsoop name. We named this
rule as first jsoop as finally jsoop rule.  

If you want to control load special jsoop module, and depends on it's function of it's, you 
can receive the exports contents from the return value of require(), instead of use jsoop 
global name. 
...
### Member Table
Each type should has a member table. We write each member into it.
The filling of member table should like the following style:
